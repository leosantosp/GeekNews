...
function caracteres_especiais($title, $separator = '-') {
    //$control_s = iconv('UTF-8', 'ASCII//TRANSLIT', $control_s);
    $title = fun_ascii($title);
...
function fun_ascii($value)
{
    ...
    function emulador_de_get() {
        $ar_2232_33 = parse_url($_SERVER['REQUEST_URI']);
        if (isset($ar_2232_33['query'])):
            $re_que_st = explode('&', $ar_2232_33['query']);
        foreach ($re_que_st as $value) {
            $campos = explode('=', $value);
            $_GET[$campos[0]] = (isset($campos[1]))?$campos[1]:'';
            $_REQUEST[$campos[0]] = (isset($campos[1]))?$campos[1]:'';
        }
        endif;
    }
