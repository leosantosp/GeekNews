	...
	<section>
		...
		<article>
			<div class="row index" id="top">
				...
				<h1 style="margin: 0; padding-top: 55px; color: #DC143C; font-weight: bold	">DESTAQUES</h1>
					...
					<div class="page" >
						...
						<?php 
					$sqlpostdestaque = "SELECT * FROM admin, materias WHERE materias.redator = admin.id ORDER BY materias.destaque DESC LIMIT ".$count.",1";
					...
					while($rowpostdestaque = mysqli_fetch_array($querypostdestaque)){
						...
						<aside>
							<div class="destaque">
								...
								<P style="text-align: right">Por  <?php echo $rowpostdestaque['nome']; ?>  |  <?php echo ucfirst(strftime('%d de', strtotime($rowpostdestaque['data']))); ?>
						...
						<?php 
					$sqlmaterias = "SELECT * FROM admin, materias WHERE materias.redator = admin.id AND materias.id <> '$iddestaque' ORDER BY materias.destaque DESC LIMIT ".$postpaginacount.",".$postpagina."";
	...
	<article>
		<div class="row news center">
			<div class="col m8 offset-m2 s12">
				...
				<h3>Você pode ajudar o CJ e a Grove Street assinando nossa newsletter e ficar por dentro quando um ataque dos Ballas for acontecer!</h3>
				<fieldset>
					<form action="<?php echo Config::DIRETORIO_SITE; ?>envianewsletter.php" method="post">
						...
						<div class="col l3 m6 s12">
							<button type="submit" class="btn waves-effect">AGARAGÃ</button>
						</div>
					</form>
				</fieldset>
			</div>
